...
//         try {
//             const healthCheck = await fetch(`${API_BASE_URL}/api/debug/tasks-connection`);
...
//                         ${error.message}
//                     </div>
//                     <button class="btn btn-primary" onclick="debugTasksLoading()" style="margin-top: 16px;">
//                         Диагностика
//                     </button>
//                     <button class="btn btn-secondary" onclick="loadTasks()" style="margin-top: 8px;">
//                         Попробовать снова
//                     </button>
//                 </div>
//             `;
...
//     }
// });
//         // 🔧 ИСПРАВЛЕННАЯ ФУНКЦИЯ ОБНОВЛЕНИЯ ПРОГРЕССА УРОВНЯ
...
//     const tasksForCurrentLevel = completedTasks % 10; // 10 заданий на уровень
//     const progressPercent = (tasksForCurrentLevel / 10) * 100;
...
//     if (levelCount) {
//         levelCount.textContent = `${tasksForCurrentLevel}/10 заданий`;
//     }
    
//     if (levelInfo) {
//         const nextLevel = currentLevel + 1;
//         const tasksNeeded = 10 - tasksForCurrentLevel;
//         levelInfo.textContent = `Выполните ${tasksNeeded} заданий для достижения ${nextLevel} уровня`;
//     }
...
// // УЛУЧШЕННАЯ ФУНКЦИЯ ОДОБРЕНИЯ ЗАДАНИЯ
...
//             apiUrl: `${API_BASE_URL}/api/admin/task-verifications/${currentVerificationId}/approve`
//         });
...
//         if (result.success) {
//             let message = `✅ Задание одобрено! Пользователь получил ${result.amountAdded}⭐`;
...
//             errorMessage = '❌ Endpoint не найден. Сообщите администратору.';
...
// // 🔧 ИСПРАВЛЕННАЯ ФУНКЦИЯ ОБНОВЛЕНИЯ ПРОГРЕССА УРОВНЯ БЕЗ БОНУСОВ
// function updateLevelProgress() {
//     if (!currentUser) return;
...
//         currentProgress: levelInfo.currentLevelTasks,
//         nextLevel: levelInfo.tasksForNextLevel,
...
function createTaskCardWithImage(task, category, index) {
    ...
    switch(category) {
        ...
        case 'completed':
            ...
            break;
        case 'rejected':
            buttonHtml = createRejectedTaskHTML(task);
            break;
    }
...
// 🔧 ПЕРЕКЛЮЧЕНИЕ КАТЕГОРИЙ ЗАДАНИЙ
function showTaskCategory(category) {
    console.log('🔄 Switching to category:', category);
