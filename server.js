...
// Добавляем новые эндпоинты для системы проверки заданий

// Эндпоинт для отправки задания на проверку
app.post('/api/user/tasks/submit', (req, res) => {
    const { userTaskId, screenshotUrl } = req.body;
    
    if (!userTaskId || !screenshotUrl) {
        return res.status(400).json({
            success: false,
            error: 'Missing required fields'
        });
    ...
    // Обновляем задание пользователя - добавляем скриншот и меняем статус
...
// Эндпоинт для получения заданий на проверку (для админа)
app.get('/api/admin/tasks/pending', (req, res) => {
    ...
    const query = `
        SELECT ut.*, up.first_name, up.last_name, up.username, up.photo_url,
               t.title, t.description, t.price, t.category
        FROM user_tasks ut
...
// Эндпоинт для одобрения задания
app.post('/api/admin/tasks/approve', (req, res) => {
    const { adminId, userTaskId } = req.body;
    ...
    db.get(`SELECT ut.*, t.price, ut.user_id 
            FROM user_tasks ut 
            JOIN tasks t ON ut.task_id = t.id 
            WHERE ut.id = ?`, [userTaskId], (err, task) => {
        if (err) {
            return res.status(500).json({
                success: false,
                error: 'Database error'
        ...
        db.run(`UPDATE user_tasks SET status = 'completed', completed_at = CURRENT_TIMESTAMP WHERE id = ?`,
                [userTaskId], function(err) {
            if (err) {
                return res.status(500).json({
                    success: false,
                    error: 'Database error'
            ...
            db.run(`UPDATE user_profiles 
                ...
                db.get("SELECT experience, level FROM user_profiles WHERE user_id = ?", 
                       [task.user_id], (err, user) => {
                    if (!err && user) {
                        const newLevel = Math.floor(user.experience / 100);
                        if (newLevel > user.level) {
                            db.run("UPDATE user_profiles SET level = ? WHERE user_id = ?", 
                                   [newLevel, task.user_id]);
                        }
                    }
                });
...
// Эндпоинт для отклонения задания
app.post('/api/admin/tasks/reject', (req, res) => {
    const { adminId, userTaskId } = req.body;
    ...
    // Получаем информацию о задании
    db.get("SELECT user_id FROM user_tasks WHERE id = ?", [userTaskId], (err, task) => {
        if (err) {
            return res.status(500).json({
                success: false,
                error: 'Database error'
        ...
        // Обновляем статус задания на rejected
        db.run(`UPDATE user_tasks SET status = 'rejected' WHERE id = ?`,
                [userTaskId], function(err) {
            if (err) {
                return res.status(500).json({
                    success: false,
...
// Эндпоинт для отмены задания пользователем
app.delete('/api/user/tasks/:userTaskId', (req, res) => {
    ...
    db.get("SELECT user_id FROM user_tasks WHERE id = ?", [userTaskId], (err, task) => {
        ...
        if (!task || task.user_id !== parseInt(userId)) {
            return res.status(403).json({
                success: false,
                error: 'Access denied'
            });
