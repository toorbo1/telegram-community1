...
bot.onText(/\/search_user (.+)/, async (msg, match) => {
    ...
    console.log('🔍 Search user command:', { userId, searchQuery });
        ...
        result.users.forEach((user, index) => {
            ...
            const userButtons = [
                [
                    {
                        text: '👀 Детали',
                        callback_data: `user_details_${user.user_id}`
                    },
                    {
                        text: user.is_blocked ? '🔓 Разблокировать' : '🚫 Заблокировать',
                        callback_data: `toggle_block_${user.user_id}`
                    }
                ],
                [
                    {
                        text: '💰 +10⭐',
                        callback_data: `add_balance_${user.user_id}_10`
                    },
                    {
                        text: '💰 -10⭐', 
                        callback_data: `subtract_balance_${user.user_id}_10`
                    }
                ],
                [
                    {
                        text: '💰 +50⭐',
                        callback_data: `add_balance_${user.user_id}_50`
                    },
                    {
                        text: '💰 -50⭐',
                        callback_data: `subtract_balance_${user.user_id}_50`
                    }
                ]
            ];
            ...
            message = ''; // Сбрасываем сообщение для следующего пользователя
        });
...
bot.on('callback_query', async (callbackQuery) => {
    ...
    try {
        ...
        if (data.startsWith('user_details_')) {
            ...
            const response = await fetch(`${APP_URL}/api/bot/user/${targetUserId}/details?adminId=${userId}`);
            const result = await response.json();
            ...
            const referralEarned = user.referral_earned || 0;
            
            let detailsMessage = `👤 <b>Детальная информация о пользователе</b>\n\n`;
            detailsMessage += `<b>Основная информация:</b>\n`;
            detailsMessage += `• Имя: ${user.first_name || 'Не указано'}\n`;
            detailsMessage += `• Фамилия: ${user.last_name || 'Не указана'}\n`;
            detailsMessage += `• Юзернейм: @${user.username || 'не указан'}\n`;
            detailsMessage += `• ID: <code>${user.user_id}</code>\n`;
            detailsMessage += `• Баланс: ${user.balance || 0}⭐\n`;
            detailsMessage += `• Регистрация: ${new Date(user.created_at).toLocaleString('ru-RU')}\n`;
            detailsMessage += `• Статус: ${user.is_blocked ? '🚫 Заблокирован' : '✅ Активен'}\n\n`;
            
            detailsMessage += `<b>Статистика заданий:</b>\n`;
            detailsMessage += `• Выполнено: ${completedTasks} ✅\n`;
            detailsMessage += `• Отклонено: ${rejectedTasks} ❌\n`;
            detailsMessage += `• На проверке: ${pendingTasks} ⏳\n`;
            detailsMessage += `• Всего заданий: ${user.total_tasks || 0}\n`;
            detailsMessage += `• Заработано: ${totalEarned.toFixed(2)}⭐\n\n`;
            
            detailsMessage += `<b>Финансовая статистика:</b>\n`;
            detailsMessage += `• Запросов на вывод: ${user.withdrawal_requests || 0}\n`;
            detailsMessage += `• Выведено средств: ${totalWithdrawn.toFixed(2)}⭐\n\n`;
            
            detailsMessage += `<b>Реферальная система:</b>\n`;
            detailsMessage += `• Приглашено: ${referralCount} пользователей\n`;
            detailsMessage += `• Заработано: ${referralEarned.toFixed(2)}⭐\n`;
            
            if (user.referrer_username) {
                detailsMessage += `• Приглашен: @${user.referrer_username}\n`;
            }
            ...
            const controlButtons = [
                [
                    {
                        text: user.is_blocked ? '🔓 Разблокировать' : '🚫 Заблокировать',
                        callback_data: `toggle_block_${targetUserId}`
                    }
                ],
                [
                    {
                        text: '💰 Управление балансом',
                        callback_data: `balance_control_${targetUserId}`
                    }
                ],
                [
                    {
                        text: '📊 Последние задания',
                        callback_data: `recent_tasks_${targetUserId}`
                    },
                    {
                        text: '💳 История выводов',
                        callback_data: `withdrawal_history_${targetUserId}`
                    }
                ]
            ];
        ...
        // Обработка блокировки/разблокировки
            ...
            const targetUserId = data.replace('toggle_block_', '');
            
            const response = await fetch(`${APP_URL}/api/bot/user/${targetUserId}/toggle-block`, {
                method: 'POST',
                headers: {
        ...
        // Обработка операций с балансом
        else if (data.startsWith('add_balance_') || data.startsWith('subtract_balance_')) {
            const parts = data.split('_');
            const operation = parts[0]; // add или subtract
            const targetUserId = parts[2];
            const amount = parts[3];
            
            const operationType = operation === 'add' ? 'add' : 'subtract';
            
            const response = await fetch(`${APP_URL}/api/bot/user/${targetUserId}/update-balance`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
            ...
            if (result.success) {
                await bot.answerCallbackQuery(callbackQuery.id, {
                    text: `Баланс обновлен: ${result.newBalance}⭐`
                });
        ...
        // Управление балансом (расширенное)
        else if (data.startsWith('balance_control_')) {
            const targetUserId = data.replace('balance_control_', '');
            
            const controlMessage = `💰 <b>Управление балансом пользователя</b>\n\n` +
                                `Выберите операцию для пользователя ID: <code>${targetUserId}</code>`;
            
            const balanceButtons = [
                [
                    {
                        text: '➕ Добавить 10⭐',
                        callback_data: `add_balance_${targetUserId}_10`
                    },
                    {
                        text: '➖ Вычесть 10⭐',
                        callback_data: `subtract_balance_${targetUserId}_10`
                    }
                ],
                [
                    {
                        text: '➕ Добавить 50⭐',
                        callback_data: `add_balance_${targetUserId}_50`
                    },
                    {
                        text: '➖ Вычесть 50⭐', 
                        callback_data: `subtract_balance_${targetUserId}_50`
                    }
                ],
                [
                    {
                        text: '➕ Добавить 100⭐',
                        callback_data: `add_balance_${targetUserId}_100`
                    },
                    {
                        text: '➖ Вычесть 100⭐',
                        callback_data: `subtract_balance_${targetUserId}_100`
                    }
                ],
                [
                    {
                        text: '🎯 Установить точную сумму',
                        callback_data: `set_balance_${targetUserId}`
                    }
                ],
                [
                    {
                        text: '🔙 Назад к деталям',
                        callback_data: `user_details_${targetUserId}`
                    }
                ]
            ];
            ...
            await bot.editMessageText(
                controlMessage,
                {
                    ...
                    parse_mode: 'HTML',
                    reply_markup: {
                        inline_keyboard: balanceButtons
                    }
                }
            );
        }
        
        // Показать последние задания пользователя
        else if (data.startsWith('recent_tasks_')) {
            const targetUserId = data.replace('recent_tasks_', '');
            ...
            let tasksMessage = `📊 <b>Последние задания пользователя</b>\n\n`;
            
            if (result.recentTasks.length === 0) {
                tasksMessage += `❌ Пользователь еще не выполнял задания.`;
            } else {
                result.recentTasks.forEach((task, index) => {
                    const statusEmoji = {
                        'completed': '✅',
                        'rejected': '❌',
                        'pending_review': '⏳',
                    ...
                    tasksMessage += `${index + 1}. ${statusEmoji} <b>${task.title}</b>\n`;
                    tasksMessage += `   💰 ${task.price}⭐ | ${task.category}\n`;
                    tasksMessage += `   📅 ${new Date(task.started_at).toLocaleDateString('ru-RU')}\n\n`;
                });
            }
        ...
        // Показать историю выводов
        else if (data.startsWith('withdrawal_history_')) {
            const targetUserId = data.replace('withdrawal_history_', '');
            ...
            let withdrawalMessage = `💳 <b>История выводов пользователя</b>\n\n`;
            
            if (result.withdrawalHistory.length === 0) {
                withdrawalMessage += `❌ Пользователь еще не делал запросы на вывод.`;
            } else {
                result.withdrawalHistory.forEach((withdrawal, index) => {
                    const statusEmoji = {
                        'completed': '✅',
                        'pending': '⏳',
                        'cancelled': '❌'
                    ...
                    withdrawalMessage += `${index + 1}. ${statusEmoji} <b>${withdrawal.amount}⭐</b>\n`;
                    withdrawalMessage += `   📅 ${new Date(withdrawal.created_at).toLocaleDateString('ru-RU')}\n`;
                    withdrawalMessage += `   Статус: ${withdrawal.status}\n\n`;
                });
            }
        ...
        console.error('User management callback error:', error);
        await bot.answerCallbackQuery(callbackQuery.id, {
...
bot.onText(/\/user_help/, async (msg) => {
    ...
    const helpMessage = `👥 <b>Управление пользователями - Справочник</b>\n\n` +
                      `<b>Основные команды:</b>\n` +
                      `<code>/search_user username</code> - Поиск пользователя по юзернейму\n` +
                      `<code>/search_user 123456</code> - Поиск по ID пользователя\n\n` +
                      
                      `<b>Функционал управления:</b>\n` +
                      `• 🔍 Поиск пользователей по юзернейму или ID\n` +
                      ...
                      `<code>/search_user john</code> - найдет всех пользователей с "john" в юзернейме`;
        ...
        helpMessage,
        {
            ...
            reply_markup: {
                inline_keyboard: [
                    [
                        {
                            text: '🔍 Попробовать поиск',
                            switch_inline_query_current_chat: '/search_user '
                        }
                    ]
                ]
            }
...
bot.on('callback_query', async (callbackQuery) => {
    ...
    try {
        ...
        if (data === 'send_notification') {
            ...
            // Добавьте этот блок в существующий обработчик:
    if (data === 'user_management') {
        ...
        await bot.sendMessage(
            ...
            '<code>/search_user username</code>\n\n' +
            'Или посмотрите справку:\n' +
            '<code>/user_help</code>',
            {
                parse_mode: 'HTML'
            }
...
app.get('/api/bot/search-users', async (req, res) => {
    ...
    console.log('🔍 Bot user search request:', { username, adminId });
    
    if (!username || !adminId) {
        return res.status(400).json({
            success: false,
            error: 'Username and adminId are required'
        });
    }
    ...
    try {
        ...
        if (!isAdmin) {
            return res.status(403).json({
                success: false,
                error: 'Доступ запрещен. Только администраторы могут искать пользователей.'
            });
        }
        ...
        console.log(`✅ Found ${usersResult.rows.length} users for search: ${username}`);
        ...
        console.error('❌ Bot user search error:', error);
        res.status(500).json({
            success: false,
            error: 'Database error: ' + error.message
        });
...
app.get('/api/bot/user/:userId/details', async (req, res) => {
    ...
    console.log('📊 Bot user details request:', { userId, adminId });
    ...
    try {
        ...
        // Получаем последние задания пользователя
...
app.post('/api/bot/user/:userId/toggle-block', async (req, res) => {
    const userId = req.params.userId;
    const { adminId } = req.body;
    
    console.log('🔒 Toggle user block request:', { userId, adminId });
    ...
    try {
        ...
        const currentStatus = userResult.rows[0].is_blocked || false;
        ...
        console.log(`✅ User ${userId} block status changed to: ${newStatus}`);
        ...
        if (bot && newStatus) {
            try {
                ...
                console.log('⚠️ Could not send block notification:', botError.message);
            }
        }
        
        res.json({
            success: true,
            message: `Пользователь ${newStatus ? 'заблокирован' : 'разблокирован'}`,
            isBlocked: newStatus
        });
...
app.post('/api/bot/user/:userId/update-balance', async (req, res) => {
    const userId = req.params.userId;
    const { adminId, amount, operation, reason } = req.body;
    
    console.log('💰 Update user balance request:', { userId, adminId, amount, operation });
    
    if (!adminId || !amount || !operation) {
        return res.status(400).json({
            success: false,
            error: 'AdminId, amount and operation are required'
        });
    }
    ...
    try {
        ...
        await pool.query(`
            ...
            (user_id, admin_id, operation_type, amount, old_balance, new_balance, reason) 
            VALUES ($1, $2, $3, $4, $5, $6, $7)
        `, [userId, adminId, operation, amountNum, currentBalance, newBalance, reason || 'Административное изменение']);
        ...
        if (bot) {
            try {
                ...
                switch (operation) {
                    case 'add':
                        operationText = `начислено ${amountNum}⭐`;
                        emoji = '📈';
                        break;
                    case 'subtract':
                        operationText = `списано ${amountNum}⭐`;
                        emoji = '📉';
                        break;
                    case 'set':
                        operationText = `установлен новый баланс: ${amountNum}⭐`;
                        emoji = '⚡';
                        break;
                }
                ...
                console.log('⚠️ Could not send balance notification:', botError.message);
            }
        }
        
        res.json({
            success: true,
            message: `Баланс пользователя обновлен: ${newBalance}⭐`,
...
app.post('/api/admin/task-verifications/:verificationId/approve', async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            // 2. Начисляем вознаграждение пользователю
            await client.query(
            ...
            if (referrerResult.rows.length > 0 && referrerResult.rows[0].referred_by) {
                ...
                if (referrerCheck.rows.length > 0) {
                    ...
                    const bonusAmount = Math.round(taskPrice * 0.1); // 10% от стоимости задания
                    
                    console.log('👥 Referral bonus calculation:', {
                    ...
                    console.log('✅ Referral bonus awarded:', referralBonus);
                }
            }

            // 5. Проверяем, достигнут ли лимит выполнения задания
            ...
            if (taskUpdateResult.rows.length > 0) {
                ...
                // Если задание выполнено нужное количество раз, помечаем его как завершенное
                if (completedCount >= peopleRequired) {
                    await client.query(
            ...
            // Добавляем информацию о реферальном бонусе, если он был начислен
            if (referralBonus) {
