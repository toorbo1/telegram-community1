...
function initDatabase() {
    db.serialize(() => {
        ...
        // In initDatabase() function, update the user_profiles table:
db.run(`CREATE TABLE IF NOT EXISTS user_profiles (
    user_id INTEGER PRIMARY KEY,
    username TEXT,
    first_name TEXT,
    last_name TEXT,
    photo_url TEXT,
    balance INTEGER DEFAULT 0, // Changed to INTEGER for stars
    level INTEGER DEFAULT 0,
    experience INTEGER DEFAULT 0,
    tasks_completed INTEGER DEFAULT 0,
    active_tasks INTEGER DEFAULT 0,
    quality_rate REAL DEFAULT 100,
    referral_count INTEGER DEFAULT 0,
    referral_earned INTEGER DEFAULT 0, // Changed to stars
    referral_code TEXT UNIQUE, // Add referral code
    referred_by INTEGER, // Track who referred this user
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP
)`);
    });
...
app.get('/api/user/:userId/referral', (req, res) => {
    const userId = req.params.userId;
    
    db.get("SELECT referral_code FROM user_profiles WHERE user_id = ?", [userId], (err, row) => {
        if (err) {
        ...
        if (!row || !row.referral_code) {
            ...
            db.run("UPDATE user_profiles SET referral_code = ? WHERE user_id = ?", 
                   [referralCode, userId], function(updateErr) {
                if (updateErr) {
            ...
            res.json({ success: true, referral_code: row.referral_code });
        }
    });
});
...
app.post('/api/user/auth', (req, res) => {
    const { user } = req.body;
    const { ref } = req.query; // Referral code from query parameter
    ...
    db.get("SELECT * FROM user_profiles WHERE user_id = ?", [userProfile.user_id], (err, existingUser) => {
        if (err) {
        ...
        if (existingUser) {
            ...
            return res.json({
                ...
                user: {
                    ...
                    experience: existingUser.experience || 0,
                    tasks_completed: existingUser.tasks_completed || 0,
                    active_tasks: existingUser.active_tasks || 0,
                    quality_rate: existingUser.quality_rate || 0,
                    referral_count: existingUser.referral_count || 0,
                    referral_earned: existingUser.referral_earned || 0
                }
            });
        }
        ...
        if (ref) {
            ...
            db.get("SELECT user_id FROM user_profiles WHERE referral_code = ?", [ref], (err, referrer) => {
                if (err) {
        ...
        function createNewUser(referredByUserId) {
            const referralCode = generateReferralCode();
            
            db.run(`INSERT INTO user_profiles 
                    (user_id, username, first_name, last_name, photo_url, referral_code, referred_by) 
                    VALUES (?, ?, ?, ?, ?, ?, ?)`,
                    [userProfile.user_id, userProfile.username, userProfile.first_name, 
                     userProfile.last_name, userProfile.photo_url, referralCode, referredByUserId],
                    function(err) {
                if (err) {
                ...
                if (referredByUserId) {
                    // Add 15 stars to referrer's balance
                    ...
                    // Optional: Add notification for referrer
                    console.log(`User ${referredByUserId} earned 15 stars for referring ${userProfile.user_id}`);
                }
...
app.get('/api/user/:userId/referral/stats', (req, res) => {
    ...
    db.get(`SELECT 
                referral_count,
                referral_earned,
                (SELECT COUNT(*) FROM user_profiles WHERE referred_by = ?) as successful_referrals
        ...
        res.json({
            success: true,
            stats: stats || { referral_count: 0, referral_earned: 0, successful_referrals: 0 }
        });
    });
});
