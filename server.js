...
// 🔧 ПРОСТАЯ ФУНКЦИЯ: Проверка прав администратора
function isUserAdmin(userId) {
    return parseInt(userId) === 8036875641;
}

// 🔧 СОЗДАНИЕ ЗАДАНИЯ (только для админа)
...
app.post('/api/admin/add-admin', async (req, res) => {
    ...
    try {
        ...
        res.json({
            success: true,
            message: `Пользователь @${user.username} добавлен как администратор`
        });
...
app.post('/api/admin/remove-admin', async (req, res) => {
    ...
    if (parseInt(targetAdminId) === 8036875641) {
        return res.status(400).json({
            success: false,
...
// 🔧 СПИСОК АДМИНОВ (только для главного админа)
app.get('/api/admin/admins-list', async (req, res) => {
    const { adminId } = req.query;
