...
// Обновите endpoint для создания постов
app.post('/api/posts', upload.single('postImage'), (req, res) => {
    const { title, content, author, authorId } = req.body;
    const image_url = req.file ? `/uploads/${req.file.filename}` : null;
    ...
    db.run(`INSERT INTO posts (title, content, author, authorId, isAdmin, image_url, likes, dislikes) 
            VALUES (?, ?, ?, ?, 1, ?, 0, 0)`,
            [title, content, author, authorId, image_url],
            function(err) {
        if (err) {
...
// Обновите endpoint для создания заданий
app.post('/api/tasks', upload.single('taskImage'), (req, res) => {
    const { 
        title, description, price, created_by, category,
...
// Добавьте таблицу для лайков/дизлайков
...
app.post('/api/posts/:postId/like', (req, res) => {
    ...
    db.get(`SELECT * FROM post_reactions WHERE post_id = ? AND user_id = ?`, 
           [postId, userId], (err, existingReaction) => {
        if (err) {
            return res.status(500).json({
                success: false,
                error: 'Database error'
        ...
        if (existingReaction) {
            ...
            if (existingReaction.reaction_type === 'like') {
                db.run(`DELETE FROM post_reactions WHERE post_id = ? AND user_id = ?`, 
                    ...
                    db.run(`UPDATE posts SET likes = likes - 1 WHERE id = ?`, [postId], function(err) {
                        ...
                        db.get(`SELECT likes, dislikes FROM posts WHERE id = ?`, [postId], (err, post) => {
                            res.json({
                                success: true,
                                likes: post.likes,
                ...
                db.run(`UPDATE post_reactions SET reaction_type = 'like' WHERE post_id = ? AND user_id = ?`,
                       [postId, userId], function(err) {
                    if (err) {
                        return res.status(500).json({
                            success: false,
                    ...
                    // Обновляем счетчики
                    db.run(`UPDATE posts SET likes = likes + 1, dislikes = dislikes - 1 WHERE id = ?`, 
                           [postId], function(err) {
                        if (err) {
                            return res.status(500).json({
                                success: false,
            ...
            // Новой лайк
            db.run(`INSERT INTO post_reactions (post_id, user_id, reaction_type) VALUES (?, ?, 'like')`,
                   [postId, userId], function(err) {
                if (err) {
                    return res.status(500).json({
                        success: false,
...
app.post('/api/posts/:postId/dislike', (req, res) => {
    ...
    db.get(`SELECT * FROM post_reactions WHERE post_id = ? AND user_id = ?`, 
        ...
        if (existingReaction) {
            // Если уже есть дизлайк - убираем его
            if (existingReaction.reaction_type === 'dislike') {
                db.run(`DELETE FROM post_reactions WHERE post_id = ? AND user_id = ?`, 
                    ...
                    // Обновляем счетчик в постах
                    db.run(`UPDATE posts SET dislikes = dislikes - 1 WHERE id = ?`, [postId], function(err) {
                        if (err) {
                            return res.status(500).json({
                                success: false,
                                error: 'Database error'
                ...
                db.run(`UPDATE post_reactions SET reaction_type = 'dislike' WHERE post_id = ? AND user_id = ?`,
                       [postId, userId], function(err) {
                    if (err) {
                        return res.status(500).json({
                            success: false,
            ...
            // Новый дизлайк
            db.run(`INSERT INTO post_reactions (post_id, user_id, reaction_type) VALUES (?, ?, 'dislike')`,
                   [postId, userId], function(err) {
                if (err) {
                    return res.status(500).json({
                        success: false,
...
// Получение реакции пользователя на пост
app.get('/api/posts/:postId/user-reaction/:userId', (req, res) => {
    const { postId, userId } = req.params;
    
    db.get(`SELECT reaction_type FROM post_reactions WHERE post_id = ? AND user_id = ?`,
           [postId, userId], (err, reaction) => {
        if (err) {
            return res.status(500).json({
                success: false,
                error: 'Database error'
...
// Обновите получение постов чтобы включать реакции
app.get('/api/posts', (req, res) => {
    db.all("SELECT * FROM posts ORDER BY timestamp DESC", (err, rows) => {
        if (err) {
            return res.status(500).json({
